...
@interface SSH_GRBResultPayView ()
...
@implementation SSH_GRBResultPayView

+ (instancetype)payView {
    
    SSH_GRBResultPayView *instance = [[SSH_GRBResultPayView alloc] init];
...
- (void)setupUI {
    ...
    // 密码输入框
    self.sonPutView = [[SSH_GRBPayPsdInputView alloc] init];
    self.sonPutView.layer.borderWidth = 0.5;
    self.sonPutView.layer.borderColor = [UIColor colorWithRGBValue:0xdbdbdb andAlpha:1.0].CGColor;
    self.sonPutView.layer.masksToBounds = YES;
    self.sonPutView.layer.cornerRadius = 10;
    [self addSubview:self.sonPutView];
    
    // 关闭按钮
    self.closeBtn = [[UIButton alloc] init];
    [self.closeBtn setImage:[UIImage imageNamed:@"jiantou_zuo_heise"] forState:UIControlStateNormal];
    ...
    self.hengLine.backgroundColor = [UIColor colorWithRGBValue:0xdbdbdb andAlpha:1.0];
...
- (void)layoutSubviews {
    ...
    [self.sonPutView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.hengLine.mas_bottom).offset(25);
...
//        make.bottom.equalTo(self.hengLine.mas_top).offset(-15);
    ...
    [self.forgetPswBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.sonPutView.mas_bottom).offset(0);
        make.height.mas_equalTo(44);
    }];
