//
//  DENGFANGLocationViewController.m
//  DENGFANGSC
//
...
@interface SSH_DingWeiViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate,UITextFieldDelegate>
...
@implementation SSH_DingWeiViewController
...
//    NSLog(@"path == %@",[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject]);
    ...
    //获取本地存储的城市
    [self.zuiDuoCityArray addObjectsFromArray:[NSKeyedUnarchiver unarchiveObjectWithFile:chengShiXuanZeFilePath]];
    if (self.zuiDuoCityArray.count == 0) { //为空的时候显示w全国
        [self.zuiDuoCityArray addObject:self.oneCityM];
    }
...
//        [MBProgressHUD showSuccess:@"选择了全国则不能选择其他城市"];
        //清除已经显示的城市，显示 全国
        [self.zuiDuoCityArray removeAllObjects];
...
-(void)clickQueDingButton{
    ...
    //存储数据
    [NSKeyedArchiver archiveRootObject:self.zuiDuoCityArray toFile:chengShiXuanZeFilePath];
    
    //传最前面的一个给首页控制器刷新数据
    DENGFANGLocationCity *citys = [self.zuiDuoCityArray firstObject];
...
//        [[NSUserDefaults standardUserDefaults] setObject:string forKey:@"chooseCitys"];
//        [[NSUserDefaults standardUserDefaults] synchronize];
    
        NSString *str = self.zuiDuoCityArray.count==1?citys.name:[NSString stringWithFormat:@"%@...",citys.name];
    if ([_delegate respondsToSelector:@selector(sl_cityListSelectedCity:Id:displayCity:cityArray:CityArrayString:)]) {
        [self.delegate sl_cityListSelectedCity:citys.name Id:citys.Id displayCity:str cityArray:self.zuiDuoCityArray CityArrayString:string];
    }
