...
@interface SSH_TouXiangXuanZeButton ()<UIActionSheetDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UIScrollViewDelegate>
...
@implementation SSH_TouXiangXuanZeButton
...
+(instancetype)iconWithFrame:(CGRect)frame cornerRadius:(CGFloat)cornerRadius image:(NSString *)image placeholderImage:(NSString *)placeholderImage completion:(IconBlock)finishBlock{
    SSH_TouXiangXuanZeButton *button = [SSH_TouXiangXuanZeButton buttonWithType:UIButtonTypeCustom];
    button.frame = frame;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info{
    ...
    UIImage *icon1 = [self scaleFromImage:icon toSize:CGSizeMake(80, 80)];  // 缩小尺寸，视实际需求而定
//    [self setBackgroundImage:icon forState:0];
    NSData *dataImage = UIImagePNGRepresentation(icon1);
    if (self.block) {  // 回调icon
        self.block(icon1,dataImage);
    }
    [[NSUserDefaults standardUserDefaults] setObject:dataImage forKey:icon_UD];
