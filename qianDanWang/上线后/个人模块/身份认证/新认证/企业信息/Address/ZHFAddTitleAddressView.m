...
-(UIView *)initAddressView{
    //初始化本地数据（如果是网络请求请注释掉-----
    NSString *imagePath = [[NSBundle mainBundle] pathForResource:self.fileName ofType:@"plist"];
    NSDictionary *dic = [[NSDictionary alloc] initWithContentsOfFile:imagePath];
...
-(void)getAddressMessageDataAddressID:(NSInteger)addressID  provinceIdOrCityId: (NSString *)provinceIdOrCityId{
    if (addressID == 1) {
        ...
        for (NSDictionary *dic in _resultArr) {
            NSString *str = self.titleMarr[0];
            if ([str isEqualToString:dic[@"name"]]) {
                [self caseCityArr:dic[@"sub"] withSelectedID:provinceIdOrCityId];
            }
        }
    } else if(addressID == 3){
        for (NSDictionary *dic in _resultArr) {
            ...
            if ([str isEqualToString:dic[@"name"]]) {
                NSArray *sub = dic[@"sub"];
                for (NSDictionary *dicA in sub) {
                    NSString *strA = self.titleMarr[1];
                    if ([strA isEqualToString:dicA[@"name"]]) {
                        [self caseCountyArr:dicA[@"sub"] withSelectedID:provinceIdOrCityId];
                    }
                }
            }
        }
    ...
    if (provinceArr.count > 0){
        for (NSDictionary *dic in provinceArr) {
            if (![dic[@"name"] isEqualToString:@"0"]) {
                ...
                [provinceModel setValuesForKeysWithDictionary:dic1];
...
-(void)caseCityArr:(NSArray *)cityArr withSelectedID:(NSString *)selectedID{
    ...
    for (NSDictionary *dic in cityArr) {
        if (![dic[@"name"] isEqualToString:selectedID]) {
            NSDictionary * dic1 = @{@"name":dic[@"name"],
                                   @"sub":dic[@"sub"]};
            CityModel *cityModel = [[CityModel alloc] init];
            [cityModel setValuesForKeysWithDictionary:dic1];
...
-(void)caseCountyArr:(NSArray *)countyArr withSelectedID:(NSString *)selectedID{
    ...
    for (NSString *string in countyArr) {
        NSDictionary * dic1 = @{@"name":string};
        CountyModel *countyModel = [[CountyModel alloc] init];
        [countyModel setValuesForKeysWithDictionary:dic1];
