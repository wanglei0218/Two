...
@interface SSH_CategorySliderBar()<UICollectionViewDelegate, UICollectionViewDataSource>
...
@implementation SSH_CategorySliderBar
...
- (void)setSelectIndex:(NSInteger)index{
    ...
    [self adjustContentOffsetToIndex:index completeHanle:^{
        SSH_CategoryItemView *itemView = (SSH_CategoryItemView *)[self.mainCollectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]];
        itemView.contentLabel.textColor = COLOR_WITH_HEX(0xe63c3f); //[UIColor redColor]
        NSNotification *notification = [[NSNotification alloc]initWithName:RESETCOLORNOTIFICATION object:@"color" userInfo:@{@"color":ColorBlack222,@"index":@(self.currentIndex)}];//[UIColor blackColor]
...
- (void)adjustIndicateViewX:(UIScrollView *)scrollView direction:(NSString *)direction{
    ...
    if ([direction isEqualToString:@"向右"]) {
        ...
        //改变颜色
        SSH_CategoryItemView *view = [self getItemViewAtIndex:index];
        view.contentLabel.textColor = [self changeRGB:view.contentLabel.textColor changeValue:distance/scrollView.width];
        
        SSH_CategoryItemView * nextView = [self getItemViewAtIndex:index-1];
...
- (SSH_CategoryItemView *)getItemViewAtIndex:(NSInteger )index{
    SSH_CategoryItemView *view = (SSH_CategoryItemView *)[self.mainCollectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]];
    return view;
}
...
{
    if (!_indicateView) {
        _indicateView = [[UIView alloc]initWithFrame:CGRectMake([self.itemOriginXArray[_originIndex] floatValue], 44 - 2, [self.itemWidthArray [_originIndex] floatValue], 2)];
        _indicateView.backgroundColor = COLOR_WITH_HEX(0xe63c3f); //[UIColor redColor]
    }
...
- (void)initItemWidthArray{
    CGFloat originX = Orangx; //itemInteval
...
- (UICollectionView *)mainCollectionView
{
    if (!_mainCollectionView) {
        ...
        [_mainCollectionView registerNib:[UINib nibWithNibName:NSStringFromClass([SSH_CategoryItemView class]) bundle:nil] forCellWithReuseIdentifier:@"ZXCategoryItemViewIdentifier"];
...
- (UICollectionViewFlowLayout *)flowLayout
{
    if (!_flowLayout) {
        ...
        _flowLayout.sectionInset = UIEdgeInsetsMake(0, Orangx, 0, Orangx);//itemInteval
    }
    return _flowLayout;
}
...
{
    SSH_CategoryItemView *cell = (SSH_CategoryItemView *)[collectionView dequeueReusableCellWithReuseIdentifier:@"ZXCategoryItemViewIdentifier" forIndexPath:indexPath];
    cell.contentLabel.text = self.itemArray[indexPath.row];
    cell.index = indexPath.row;
    if (indexPath.row == self.currentIndex) {
        cell.contentLabel.textColor = COLOR_WITH_HEX(0xe63c3f);// [UIColor redColor]
    }
    else{
        cell.contentLabel.textColor = ColorBlack222;//[UIColor blackColor]
    }
    return cell;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[NSNotificationCenter defaultCenter]postNotification:notification];
    SSH_CategoryItemView *cell = (SSH_CategoryItemView *)[collectionView cellForItemAtIndexPath:indexPath];
    cell.contentLabel.textColor = COLOR_WITH_HEX(0xe63c3f);// [UIColor redColor]
    ...
    if ([self.delegate respondsToSelector:@selector(didSelectedIndex:)]) {
        ...
        if (indexPath.row == 0) {
            [MobClick event:@"lists-all"];
        }else if (indexPath.row == 1){
            [MobClick event:@"list-fund"];
        }else if (indexPath.row == 2){
            [MobClick event:@"list-social"];
        }else if (indexPath.row == 3){
            [MobClick event:@"list-car"];
        }else{
            [MobClick event:@"list-house"];
        }
    }
}
