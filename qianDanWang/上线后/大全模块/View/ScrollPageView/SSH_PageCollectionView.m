...
#import "SSH_PageCollectionView.h"
...
@interface SSH_PageCollectionView()<UIScrollViewDelegate>
{
...
@implementation SSH_PageCollectionView
...
- (void)setDataSource:(id<SSH_PageCollectionViewDataSource>)dataSource
{
    _dataSource = dataSource;
    _totolCount = [self.dataSource numberOfItemsInSSH_PageCollectionView:self];
...
- (UIView *)checkDataSourceViewAtIndex:(NSInteger) index{
    ...
    if ([self.dataSource respondsToSelector:@selector(SSH_PageCollectionView:viewForItemAtIndex:)]) {
        UIView *dataSourceView = [self.dataSource SSH_PageCollectionView:self viewForItemAtIndex:index];
        if (dataSourceView) {
            view = dataSourceView;
        }
    }
...
- (BOOL)viewIsInScreen:(CGRect)frame {
    ...
    CGFloat tyd_screenWidth = self.mainScrollView.frame.size.width;
    
    CGFloat contentOffsetX = self.mainScrollView.contentOffset.x;
    if (CGRectGetMaxX(frame) > contentOffsetX && x-contentOffsetX < tyd_screenWidth) {
        return YES;
    } else {
        return NO;
    }
