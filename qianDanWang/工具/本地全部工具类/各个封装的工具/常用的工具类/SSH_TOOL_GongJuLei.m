...
#import "SSH_TOOL_GongJuLei.h"

@implementation SSH_TOOL_GongJuLei
...
+ (void)showErrorAlter:(UIView *)view {
    ...
    NSArray * arr = [SSH_TOOL_GongJuLei getCurrentVC];
...
+ (NSString *)md5EncryptWithFormulaFromString:(NSString *)string{
    ...
    //JGQLog(@"最终的时间戳：%@",[DENGFANGSingletonTime shareInstance].timestamp);
...
+ (NSString *)changeTelephone:(NSString *)teleStr{
    NSString *phoneStr = [teleStr stringByReplacingCharactersInRange:NSMakeRange(3, 4) withString:@"****"];
    return phoneStr;
}
