...
+(instancetype)cycleScrollViewWithFrame:(CGRect)frame shouldInfiniteLoop:(BOOL)infiniteLoop
{
    DCCycleScrollView *cycleScrollView = [[DCCycleScrollView alloc]initWithFrame:frame];
    cycleScrollView.infiniteLoop = infiniteLoop;
    return cycleScrollView;
...
{
    return (SCREEN_WIDTH - self.itemWidth)/2;
}
-(void)layoutSubviews
{
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    NSInteger index = [self currentIndex] % self.imgArr.count;
    self.pageControl.currentPage = index;
    NSInteger lastIndex = [[NSUserDefaults standardUserDefaults] integerForKey:@"index"];
    if (index != lastIndex) {
        [[NSUserDefaults standardUserDefaults] setInteger:index forKey:@"index"];
        [[NSUserDefaults standardUserDefaults] synchronize];
        [self.delegate cycleScrollView:self endScrollerAtIndex:index];
    }
}
...
-(void)setImgArr:(NSArray *)imgArr
{
    ...
    [self.collectionView reloadData];
    if (imgArr.count) {
        [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForRow:scrollIndex inSection:0] atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:NO];
        _oldPoint = self.collectionView.contentOffset.x;
    }
    self.collectionView.userInteractionEnabled = YES;
}
